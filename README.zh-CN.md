...
运行方法、ROS 接口、真值使用边界和30种子正式评测矩阵见 [`docs/generalized-harvest-v1.md`](docs/generalized-harvest-v1.md)。当前完成的是实现与测试基础，472项回归测试全部通过，并已加入与正式种子隔离的120场景开发数据采集/训练/资格复核入口；但现有视觉模型尚未通过随机多目标开发门槛，正式矩阵未执行。只有完成开发集微调与独立复核，并实际完成全部30个一次性运行、得到 `overall_pass: true` 后，才能声称达到计划中的泛化验收指标。
