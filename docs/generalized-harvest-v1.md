...
截至 2026-08-31，当前里程碑的完整纯 Python 回归为 `750 tests, 0 failures, 0 errors, 2 skipped`，完整 ROS 2/colcon 回归为 `632 tests, 0 failures, 0 errors, 0 skipped`。无运动真实 ROS 图审计已通过：定位、跟踪、选择、编排和操作控制节点均未订阅真值话题；只有仿真适配/评分边界可以读取真值。
...
当前开发矩阵为 `config/generalized_runtime_development_matrix_v2.json`。v2 的 `45101` 与 `45104` 已用于诊断，因此整个发现块 `45101～45118` 不再作为新鲜批次；下一轮发现应使用 `--batch-index 1`，即 `45201～45218`。v2 资格块 `46101～46118` 尚未使用，只有发现问题修复并冻结代码后才能运行。先运行不执行机械臂轨迹的筛选：
...
python scripts/run_generalized_feasibility_sweep.py \
  ...
  --split discovery \
  --batch-index 1 \
  --output-dir .codex_tmp/generalized_feasibility_discovery_b01
```
