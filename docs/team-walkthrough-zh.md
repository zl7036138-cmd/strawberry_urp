...
5. 所有重大技术决策写入 **ADR**（Architecture Decision Record，`docs/decisions/`，现有 81 份，编号至 0079）。
...
- **MoveIt 预选**（ADR 0077）：目标发布前先做连通预抓取/接近/抓取/撤离可行性检查，不可行目标根本不发布（避免批次里反复失败）；
...
`docs/decisions/` 现有 81 份 ADR（编号至 0079），格式 `NNNN-简短描述.md`，每份记录**背景 → 决策 → 后果**。写论文、答辩、换人接手时直接引用。代表性：
...
- ADR 0078：真值隔离、无运动筛选和一次性资格门合同；
- ADR 0079：资格批只有 1/18 合格，因此按合同停止行为运行。
...
冻结提交 `9300ce9` 上的未调试资格种子 `46001～46018` 已完成一次性无运动筛选：
18/18 探针完成、18/18 真值隔离和清理通过，但仍只有 `46016` 一场合格。因不足五场，
系统没有启动行为运行，正式 30 种子继续封存。

### 下一步（按序）

1. 用本批证据修正随机果实布置与机械臂保守连通可行域的错配，并先增加复现测试；
2. 冻结行为改动后切换到下一个未使用的百位资格种子块，重新做无运动筛选；
3. 只有选满五场后才各执行一次连续采摘，并要求至少 4/5 同批采摘 ≥2 颗；
