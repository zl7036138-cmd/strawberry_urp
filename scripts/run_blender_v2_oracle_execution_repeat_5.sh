...
contract="${repo_root}/config/blender_v2_oracle_execution_repeat_5_v1.json"
output_dir="${repo_root}/results/development/blender_v2_oracle_execution_repeat_5_v1"
trial_count=5
...
[[ -f "${contract}" ]] || {
  echo "frozen repeat contract is missing: ${contract}" >&2
  exit 2
}
...
trap - EXIT INT TERM
python "${repo_root}/scripts/summarize_blender_v2_oracle_execution_repeat.py" \
  --contract "${contract}" \
  --results "${output_dir}" \
