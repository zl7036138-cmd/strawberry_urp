...
contract="${repo_root}/config/blender_v2_oracle_execution_round_trip_v1.json"
output_dir="${repo_root}/results/development/blender_v2_oracle_execution_round_trip_v1"
...
[[ -f "${contract}" ]] || {
  echo "frozen execution contract is missing: ${contract}" >&2
  exit 2
}
...
for candidate in $(seq 180 232); do
...
if [[ ! -f "${trial_json}" ]]; then
  printf '{"success":false,"error":"client exited with status %s without a result"}\n' \
    "${client_status}" >"${trial_json}"
fi

python "${repo_root}/scripts/validate_blender_v2_oracle_execution.py" \
  --contract "${contract}" \
  --trial "${trial_json}" \
