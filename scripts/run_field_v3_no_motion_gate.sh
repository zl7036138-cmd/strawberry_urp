...
output_dir="${1:-${repo_root}/results/development/field_v3_no_motion_v1}"
domain_id="${2:-224}"
...
setsid ros2 launch strawberry_sim field_v3.launch.py \
  headless:=true camera_mount:=dual simulation_seed:=20260728 \
  >"${output_dir}/launch.log" 2>&1 &
launch_pid=$!

timeout --signal=TERM 120 ros2 run strawberry_sim runtime_health_check \
  --duration-sec 8 --startup-timeout-sec 90 --report-interval-sec 4 \
...
timeout --signal=TERM 60 ros2 run strawberry_sim runtime_health_check \
  --duration-sec 8 --startup-timeout-sec 45 --report-interval-sec 4 \
...
trap - EXIT INT TERM
if grep -Eqi \
  "Traceback|process has died|RuntimeError|Unable to find uri|Error Code [0-9]+" \
  "${output_dir}/launch.log" \
  "${output_dir}/base_runtime_health.log" \
  "${output_dir}/wrist_runtime_health.log"; then
  echo "Field-v3 no-motion logs contain an unhandled failure" >&2
  exit 1
fi
echo "Field-v3 no-motion gate: ${output_dir}"
