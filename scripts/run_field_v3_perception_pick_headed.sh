...
[[ "${show_windows}" == "true" || "${show_windows}" == "false" ]] || {
...
setsid ros2 launch strawberry_bringup system.launch.py \
  ...
  target_refinement_topic:="${target_topic}" \
  manipulation_request_timeout_sec:=12.0 \
  manipulation_safe_transit_clearance_m:=0.02 \
  manipulation_place_transit_clearance_m:=0.12 \
...
setsid ros2 run strawberry_perception perception_node --ros-args \
  ...
  -r __node:=strawberry_field_demo_perception \
  -p use_sim_time:=true \
...
setsid ros2 run strawberry_localization localization_node --ros-args \
  ...
  -r __node:=strawberry_field_demo_localization \
  -p use_sim_time:=true \
  -p confidence_threshold:=0.31 \
  -p detections_topic:="${detections_topic}" \
...
if [[ "${show_windows}" == "true" ]]; then
  setsid python "${repo_root}/scripts/show_field_v3_perception.py" \
    --image-topic /camera/wrist/color/image_raw \
    --detections-topic "${detections_topic}" \
    --target-topic "${target_topic}" \
    --window-title "Field-v3 | Live perception pick" \
    --banner "FIELD-v3 | SIMULATION PERCEPTION PICK | ONE ACTION" \
    --roi 320 240 640 480 \
...
python "${repo_root}/scripts/evaluate_blender_v2_perception_execution_readiness.py" \
  ...
  --scene "${repo_root}/ros2_ws/src/strawberry_sim/config/scene_field_v3.yaml" \
  --geometry-gate \
...
run_pick_client() {
  timeout --signal=TERM 720 python \
    ...
    --place-x -0.45 --place-y 0.25 --place-z 0.45 \
...
if (
  [[ "${client_status}" -ne 0 ]] &&
  grep -Fq \
    "target did not become stable" \
    "${output_dir}/trial_01.json"
); then
  # DDS discovery can occasionally outlive the first short-lived subscriber
  # even though the preceding safety probes and live display are healthy.
  # This retry is allowed only before any action feedback or contact exists.
  mv \
    "${output_dir}/trial_01.json" \
    "${output_dir}/target_subscription_attempt_01.json"
  mv \
    "${output_dir}/client.log" \
    "${output_dir}/target_subscription_attempt_01.log"
  sleep 2
  run_pick_client
  client_status=$?
fi
...
echo "Field-v3 headed pick finished with status ${client_status}" \
  >"${output_dir}/demo_status.txt"
sleep "${post_demo_hold_sec}"

if [[ "${client_status}" -ne 0 ]]; then
  echo "Field-v3 perception pick failed with client status ${client_status}" >&2
  exit "${client_status}"
fi
echo "Field-v3 perception pick: ${output_dir}/trial_01.json"
